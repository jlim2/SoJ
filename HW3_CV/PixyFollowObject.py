...
"""PixyCam tutorial on EV3DEV.ORG (http://www.ev3dev.org/docs/tutorials/using-pixy-camera/#example-2---following-an-object)
Set a signature first using PixyMon
"""

def limit_speed(speed):
  """ Limit speed in range [-200,200] """
  if speed > 200:
    speed = 200
  elif speed < -200:
    speed = -200
...
while not ts.value():
  if pixy.value(0) > 0:
    ...
    # Use GAIN otherwise speed will be to slow, but limit in range [-200,200]
